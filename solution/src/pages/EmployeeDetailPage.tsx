...
import { useEmployeeGQL } from "../hooks/useEmployeeGQL";

export function EmployeeDetailPage() {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();

  // Swapped from useEmployee (TanStack + REST) to useEmployeeGQL (Apollo + GraphQL).
  const { data: employee, isLoading, isError } = useEmployeeGQL(Number(id));
